...
public class VarInstSplit implements SLAlg {
	...
	public GraphSPN learnStructure(Dataset d) {
		...
		toProcess.add(wholeTraingingset);
            ...
            try {
                fileWriter = new FileWriter("Independent_Sets.txt");
            } catch (IOException ex) {
                System.out.println("oops");
                Logger.getLogger(VarInstSplit.class.getName()).log(Level.SEVERE, null, ex);
            }
		...
		while(!toProcess.isEmpty()){
			...
			if(nextID > 1){
				...
				//				System.out.println((toc-tic)+"ms");
                                if (currentSlice.instances.length==allInstances.length){
                                    System.out.println("new independent set");
                                    try {
                                        writer.append("\n");
                                    } catch (IOException ex) {
                                        Logger.getLogger(VarInstSplit.class.getName()).log(Level.SEVERE, null, ex);
				...
				long toc = System.currentTimeMillis();
                                if (currentSlice.instances.length==allInstances.length){
                                    ...
                                    try {
                                        ...
                                        //write to file if contains 53(protected).
